...
export default function UserImportForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Import successful", {
        description: data.message || "Users imported successfully",
        cancel: {
        ...
        action: {
          ...
          onClick: () => handleApiResult({ success: true, data, message: data.message || "Users imported successfully" }, "Importar Usuarios")
        }
      });
    } catch (error: any) {
      handleApiResult(error, "Importar Usuarios");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      {/* File input */}
      <div>
        <Label htmlFor="user-file">JSON File</Label>
        <FileInputButton
          accept=".json"
      ...
      {result && (
        <div className="mt-4 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="text-sm space-y-1">
            <p>Total: {result.data?.totalItems || result.totalUsers}</p>
