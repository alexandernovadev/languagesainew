...
export function ConfigModal() {
  const { configs, currentGenerationConfig, setCurrentGenerationConfig } =
  ...
  const [config, setConfig] = useState<TranslationConfig>({
    ...
    mustUseWords: currentGenerationConfig?.mustUseWords || [],
    grammarTopics: currentGenerationConfig?.grammarTopics || [],
    topic: currentGenerationConfig?.topic || "general",
  });
  ...
  // Ensure configs are loaded before rendering
  if (!configs) {
  ...
  const handleTranslationModeChange = (modeId: string) => {
    const mode = configs.defaultConfigs.translationModes.find(
      (m) => m.id === modeId
    );
    if (mode) {
      setConfig((prev) => ({
  ...
  useEffect(() => {
    ...
    Object.entries(selectedWords).forEach(([type, wordIds]) => {
      if (type === "recent" && configs.recentWords) {
        ...
        const words = configs.userWords[
          type as keyof typeof configs.userWords
        ] as any[];
        wordIds.forEach((id) => {
    ...
    setCurrentGenerationConfig(finalConfig);
  }, [config, selectedWords, configs, setCurrentGenerationConfig]); // Add setCurrentGenerationConfig to dependency array

  const getCurrentModeId = () => {
    const mode = configs.defaultConfigs.translationModes.find(
      (m) =>
        m.sourceLanguage === config.sourceLanguage &&
        m.targetLanguage === config.targetLanguage
    );
      ...
      <Tabs defaultValue="general" className="w-full">
        ...
        <TabsContent value="general" className="space-y-4 mt-6">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setConfig((prev) => ({ ...prev, difficulty: value as any }))
              }
            >
              <SelectTrigger>
                <SelectValue />
        ...
        <TabsContent value="words" className="space-y-4 mt-6">
          <div className="text-sm text-muted-foreground mb-4">
            Select words that must be included in the generated text (5 max per
            type)
          </div>
                      ...
                      {words.map((word: any) => (
                        ...
                        >
                          <Checkbox
                            id={`${type}-${word.id}`}
                            checked={
                              selectedWords[type]?.includes(word.id) || false
                            }
                            ...
                            disabled={
                              !selectedWords[type]?.includes(word.id) &&
                              selectedWords[type]?.length >= 5
                            }
                          />
                          <label
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <Checkbox
                        id={`recent-${word.id}`}
                        checked={
                          selectedWords.recent?.includes(word.id) || false
                        }
                        onCheckedChange={(checked) =>
                          handleWordToggle(
                            "recent",
                        ...
                        disabled={
                          !selectedWords.recent?.includes(word.id) &&
                          (selectedWords.recent?.length || 0) >= 10
                        }
                      />
