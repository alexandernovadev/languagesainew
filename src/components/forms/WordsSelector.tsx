...
export function WordsSelector({ 
  ...
  const handleSelectRandom = () => {
    ...
    const allWords = Object.values(wordsByType).flat().map(w => w.word);
  ...
  const handleSelectType = (typeKey: string) => {
    ...
    const newWords = currentTypeWords.length === typeWords.length
      ? selectedWords.filter(word => !typeWords.some(w => w.word === word))
      : [...selectedWords.filter(word => !typeWords.some(w => w.word === word)), ...typeWords.map(w => w.word)];
  ...
  const isTypeSelected = (typeKey: string) => {
    ...
    return selectedTypeWords.length === typeWords.length && typeWords.length > 0;
  };
  ...
  const isTypePartiallySelected = (typeKey: string) => {
    ...
    return selectedTypeWords.length > 0 && selectedTypeWords.length < typeWords.length;
  };
  ...
  const loadWordsByType = async (typeKey: string) => {
    ...
    try {
      ...
      const words = await wordService.getWordsByTypeOptimized(typeKey, 10, searchTerm);
  ...
  const handleTypeExpand = (typeKey: string) => {
    ...
    if (!preloadedWords && (!wordsByType[typeKey] || wordsByType[typeKey].length === 0)) {
  ...
  const filteredWordsByType = (typeKey: string) => {
    ...
    return words.filter(word => 
      word && word.word && word.word.toLowerCase().includes(searchTerm.toLowerCase())
    );
  };
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    ...
                    {selectedWords.length} palabra{selectedWords.length !== 1 ? 's' : ''} seleccionada{selectedWords.length !== 1 ? 's' : ''}
                  </Badge>
                </TooltipTrigger>
                          ...
                          <div className="flex items-center gap-2 w-full">
                            ...
                            <div className="flex-1 text-left">
                              ...
                              <div className="text-xs text-muted-foreground">
                                {isLoading ? 'Cargando...' : `${selectedTypeWords.length} de ${typeWords.length} seleccionadas`}
                              </div>
                            </div>
                          </div>
                        </AccordionTrigger>
                      ...
                      <AccordionContent className="px-3 pb-3">
                        {isLoading ? (
                          <div className="flex items-center justify-center py-4">
                            ...
                            <span className="text-sm">Cargando palabras...</span>
                          </div>
                        ) : (
                              ...
                              {filteredWords.map((word, index) => (
                                ...
                                >
                                  <Checkbox
                                    ...
                                    onCheckedChange={() => handleWordToggle(word.word)}
                                ...
                                <div className="text-center py-4 text-xs text-muted-foreground">
                                  {typeWords.length === 0 ? (
                                    <div className="space-y-2">
                                      <p>No tienes palabras de este tipo en tu biblioteca.</p>
                                      <p className="text-xs">Agrega palabras desde "Mis Palabras" primero.</p>
                                    </div>
                                  ) : (
