...
export default function ExamImportForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Import successful", {
        description: data.message || "Exams imported successfully",
        cancel: {
        ...
        action: {
          ...
          onClick: () => handleApiResult({ success: true, data, message: data.message || "Exams imported successfully" }, "Importar Exámenes")
        }
      });
    } catch (error: any) {
      handleApiResult(error, "Importar Exámenes");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      {/* File input */}
      <div>
        <Label htmlFor="exam-file">JSON File</Label>
        <FileInputButton
          accept=".json"
      ...
      {result && (
        <div className="mt-4 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="text-sm space-y-1">
            <p>Total: {result.data?.totalItems || result.totalExams}</p>
