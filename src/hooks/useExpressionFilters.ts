...
export function useExpressionFilters() {
  ...
  const getActiveFiltersDescription = () => {
    ...
    if (filters.hasImage) {
      activeFilters.push(`Imagen: ${filters.hasImage === 'true' ? 'Sí' : 'No'}`);
    }
    
    if (filters.hasExamples) {
      activeFilters.push(`Ejemplos: ${filters.hasExamples === 'true' ? 'Sí' : 'No'}`);
    }
    ...
    if (filters.createdAfter || filters.createdBefore) {
      const dateRange = [];
      if (filters.createdAfter) dateRange.push(`Desde: ${filters.createdAfter}`);
      if (filters.createdBefore) dateRange.push(`Hasta: ${filters.createdBefore}`);
