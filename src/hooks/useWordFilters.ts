...
export function useWordFilters() {
  ...
  const hasActiveFilters = useMemo(() => {
    const hasBasicFilters = Object.values(filters).some(value => 
      value !== undefined && value !== "" && value !== null
    );
    const hasBooleanFilters = Object.values(booleanFilters).some(value => value === true);
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (value !== undefined && value !== "" && value !== null) {
        if (key === 'level' && typeof value === 'string') {
          ...
          descriptions.push(`${key === 'seenMin' ? 'Mín' : 'Máx'} vistas: ${value}`);
        } else if (key === 'sortBy') {
